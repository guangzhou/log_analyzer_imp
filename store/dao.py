...
def fetch_all_templates(active_only: bool = True) -> List[sqlite3.Row]:
    with _connect() as conn:
        cur = conn.execute(
            "SELECT template_id, pattern, sample_log FROM regex_template WHERE is_active=1"
            if active_only else
            "SELECT template_id, pattern, sample_log FROM regex_template"
        )
        return cur.fetchall()
...
def write_templates(cands: List[Dict[str, Any]]) -> List[int]:
    ...
    with _connect() as conn:
        for c in cands:
            ...
            cur = conn.execute("""
                INSERT INTO regex_template(pattern, sample_log, version, is_active, semantic_info, advise, created_at, updated_at, source)
                VALUES(?, ?, 1, 1, ?, ?, ?, ?, ?)
            """, (pattern, sample_log, semantic_info, advise, now, now, source))
            tid = cur.lastrowid
            ids.append(tid)
            conn.execute("""
                INSERT INTO template_history(template_id, pattern, sample_log, version, created_at, source, note)
                VALUES(?, ?, ?, 1, ?, ?, ?)
            """, (tid, pattern, sample_log, now, source, "首次创建"))
        conn.commit()
    return ids
...
def batch_upsert_log_match_summary(rows: List[Dict[str, Any]]):
    ...
    with _connect() as conn:
        for r in rows:
            conn.execute("""
                INSERT INTO log_match_summary(run_id, file_id, template_id, mod, smod, classification, level, thread_id, first_ts, last_ts, line_count, updated_at)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (r["run_id"], r["file_id"], r["template_id"], r["mod"], r["smod"], r["classification"], r.get("level",""), r.get("thread_id",""), r["first_ts"], r["last_ts"], r["line_count"], now))
        conn.commit()
...
def batch_upsert_key_time_bucket(rows: List[Dict[str, Any]]):
    ...
    with _connect() as conn:
        for r in rows:
            conn.execute("""
                INSERT INTO key_time_bucket(run_id, file_id, template_id, mod, smod, classification, level, thread_id, bucket_granularity, bucket_start, count_in_bucket, updated_at)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (r["run_id"], r["file_id"], r["template_id"], r["mod"], r["smod"], r["classification"], r.get("level",""), r.get("thread_id",""), r["bucket_granularity"], r["bucket_start"], r["count_in_bucket"], now))
        conn.commit()
...
def get_template_samples(limit: int = 200) -> List[str]:
    with _connect() as conn:
        cur = conn.execute("SELECT sample_log FROM regex_template WHERE sample_log IS NOT NULL AND sample_log != '' ORDER BY updated_at DESC LIMIT ?", (limit,))
